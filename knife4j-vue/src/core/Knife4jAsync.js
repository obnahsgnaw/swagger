  ...
  if (!swpinfo.init) {
    ...
    if (swpinfo.parameters != null) {
      ...
      if (apiInfo.consumes == undefined || apiInfo.consumes == null || apiInfo.consumes.length == 0){
        apiInfo.consumes = that.currentInstance.openApiBaseInfo.consumes
      }
      //判断consumes请求类型
      if (apiInfo.consumes != undefined && apiInfo.consumes != null && apiInfo.consumes.length > 0) {
        var ctp = apiInfo.consumes[0]; //TODO
        ...
        } else if (ctp.indexOf("application/json") >= 0) { // TODO added json
          swpinfo.contentType = ctp;
          swpinfo.contentValue = "raw";
          swpinfo.contentShowValue = "JSON(application/json)";
          swpinfo.contentMode = "json";
        } else if (ctp.indexOf("application/x-protobuf") >= 0) { // TODO added protobuf
          swpinfo.contentType = ctp;
          swpinfo.contentValue = "raw";
          swpinfo.contentShowValue = "Proto(application/x-protobuf)";
          swpinfo.contentMode = "protobuf";
        } else {
...
//  var newfullPath = ""; // TODO deleted
  ...
  if (apiInfo != null) {
    ...
    if (apiInfo.produces == undefined || apiInfo.produces == null || apiInfo.produces.length == 0){
      apiInfo.produces = that.currentInstance.openApiBaseInfo.produces
    }
